		 ...
		 for (int i=0;i<numberOfProblems;i++){
			 ...
			 for (int j=0;j<numberOfDataFiles;j++){
				 ...
				 for (int p=0;p<numberOfPastTransactions;p++){
					 ...
					 numberOfReqs = numberOfVariables/5;
					 ...
					 while(true){
						 ...
						 for (int k=0;k<numberOfReqs;k++){
							 ...
							 int index= 0;
							 do{
								 index = (int)(Math.random() * numberOfVariables);
							 ...
							 // pick a value
								 ...
								 pastReqs[p][index]=lb+value;
								 constraints[k] = IntConstraintFactory.arithm(intVars[index],"=",lb+value);
					 ...
					 if(p!=numberOfPastTransactions-1){
						  diagnoses[p] = FlexDiag.diagnose(fzn[p].getSolver(),numberOfReqs,m);
				...
				Constraint [] diagnosis_lcr = FlexDiag.diagnose(fzn[numberOfPastTransactions-1].getSolver(),pastReqs_sorted,numberOfReqs,m);
				...
				Constraint [] diagnosis_fd = FlexDiag.diagnose(fzn_fd.getSolver(),numberOfReqs,m);
				...
				fzn_fd2.parse(solver_fd2, targetStream);
				 ...
				 for(int g=0;g<numberOfVariables;g++)
				 {
					 if(pastReqs[numberOfPastTransactions-1][g]!=-1){
						 Constraint c= IntConstraintFactory.arithm(intVars_fd2[g],"=",pastReqs[numberOfPastTransactions-1][g]);
						 solver_fd2.post(c); 
					 } 
				 }
				 
				Constraint [] diagnosis_fd2 = FlexDiag.diagnose(fzn_fd2.getSolver(),numberOfReqs,1);
				
				double minimality_lcr = ((double)(diagnosis_fd2.length))/ ((double)(diagnosis_lcr.length));
			    double minimality_fd = ((double)(diagnosis_fd2.length))/ ((double)(diagnosis_fd.length));
				...
				String result = i+"#"+j+"#"+m+"#"+formatter.format(runtime_lcr_double)+"#"+formatter.format(minimality_lcr)+"#"+formatter.format(combined_lcr)+"###"+formatter.format(runtime_fd_double)+"#"+formatter.format(minimality_fd)+"#"+formatter.format(combined_fd)+"#"+improvementRatio+"\n";
